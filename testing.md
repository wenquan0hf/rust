...
那么为什么我们懒惰的测试通过呢?任何不应急的测试都可以通过，任何应急的测试都会失败。让我们来看一个失败的测试：
...
这里有一些变化。第一个是引入带有 **cfg** 属性的 **mod tests**。模块允许我们分组所有的测试，如果需要也可以定义 helper 函数，这个函数不会成为我们 crate 的一部分。如果目前我们试图运行这些代码，**cfg** 属性只会编译我们的测试代码。这可以节省编译时间，也保证了我们构建的测试是完全正常的。

第二个变化是**使用**声明。因为我们在一个内部模块中，我们需要将我们的测试函数设置范围。如果你有一个大的模块，这可能就会很恼人，所以这是 glob 属性的一种常见的使用方式。让我们改变我们的 src/lib.rs 以便能够使用它：
...
当前的惯例是使用测试模块 “unit-style” 测试。任何只测试一个小功能都是有意义的。如果用 “integration-style” 测试替代会怎么样呢?为此，我们有测试目录。
...
这类似于我们之前的测试，但略有不同。我们现在在顶部有一个 **extern crate adder**。这是因为在测试目录里测试是一个完全独立的箱，所以我们需要导入我们的函数库。这也是为什么 tests 是一个编写集成风格测试的合适的地方：他们使用函数库和其他消费者。
...
这就是所有的测试目录。这里不需要测试模块，因为整件事都是专注于测试的。
...
注意：模块级文档使用 //! 和函数文档使用 ///。Rust 的文档支持Markdown 中评论，所以三重斜线标志代码块。包含 # Examples 部分是一种惯例，以下所示。
